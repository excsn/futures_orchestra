...
impl<R: Send + 'static> TaskHandle<R> {
  ...
  pub fn cancel(&self) {
    tracing::debug!(task_id = %self.id(), "TaskHandle: Cancellation requested.");
    self.core.cancel();
  }
  ...
  pub fn detach(mut self) {
    ...
    tracing::trace!(task_id = %self.id(), "TaskHandle: Task detached.");
  }
      ...
      Some(mut rx) => {
        match rx.recv().await {
          ...
          Err(oneshot_recv_error) => {
            ...
            tracing::warn!(task_id = %self.id(), "Result channel receive error: {}", oneshot_recv_error);
