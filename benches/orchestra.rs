...
use futures_orchestra::{FuturePoolManager, FuturePoolManagerRlxd, TaskCompletionInfo, TaskLabel};
...
async fn drain_rlxd(manager: &FuturePoolManagerRlxd<u64>) -> u64 {
  let mut handles = Vec::with_capacity(TASKS as usize);
  for i in 0..TASKS {
    handles.push(
      manager
        .submit_future(HashSet::new(), async move { black_box(i) })
        .await
        .unwrap(),
    );
  }
...
fn bench_rlxd(c: &mut Criterion, name: &str, dispatchers: usize, with_handler: bool) {
  let rt = Runtime::new().unwrap();
  let manager = FuturePoolManagerRlxd::<u64>::new(CONCURRENCY, QUEUE_CAPACITY, dispatchers, rt.handle().clone(), name);
  ...
  group.bench_function(name, |b| {
    b.to_async(&rt).iter(|| {
      let manager = manager.clone();
      async move { drain_rlxd(&manager).await }
    });
  });
  group.finish();
}
...
fn rlxd_d1_no_handler(c: &mut Criterion) {
  bench_rlxd(c, "rlxd_d1_no_handler", 1, false);
}

fn rlxd_d2_no_handler(c: &mut Criterion) {
  bench_rlxd(c, "rlxd_d2_no_handler", 2, false);
}

fn rlxd_d4_no_handler(c: &mut Criterion) {
  bench_rlxd(c, "rlxd_d4_no_handler", 4, false);
}

fn rlxd_d1_with_handler(c: &mut Criterion) {
  bench_rlxd(c, "rlxd_d1_with_handler", 1, true);
}

fn rlxd_d2_with_handler(c: &mut Criterion) {
  bench_rlxd(c, "rlxd_d2_with_handler", 2, true);
}

fn rlxd_d4_with_handler(c: &mut Criterion) {
  bench_rlxd(c, "rlxd_d4_with_handler", 4, true);
}
...
criterion_group!(
  ...
  with_handler,
  labelled_no_handler,
  labelled_with_handler,
  rlxd_d1_no_handler,
  rlxd_d2_no_handler,
  rlxd_d4_no_handler,
  rlxd_d1_with_handler,
  rlxd_d2_with_handler,
  rlxd_d4_with_handler
);
